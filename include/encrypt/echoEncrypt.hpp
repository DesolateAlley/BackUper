#ifndef ECHOENCRYPT_HPP
#define ECHOENCRYPT_HPP
...
class AES {
 public:
    ...
    void divideToByte(byte out[16], std::bitset<128> &data);        // 连续128存入一个数组
    std::bitset<128> mergeByte(byte in[16]);                        // 16个byte合成128位
...
class EchoEncrypt  : public FileUtils, public MD5, public AES{
 public:
    EchoEncrypt(/* args */){};
    ~EchoEncrypt(){};

    bool encyptFile(std::string md5ofpwd, std::string md5md5ofpwd, std::string sourcefile, std::string targetfile);  // 对sourcefile进行aes加密， targetfile 为加密文件存放目录
    bool encyptAESFile( std::string sourcefile, std::string targetfile , std::string password ="123456");  // 对sourcefile进行aes加密， targetfile 为加密文件存放目录

    bool decryptFile(std::string md5ofpwd, std::string md5md5ofpwd, std::string sourcefile, std::string targetfile); // 对sourcefile进行aes解密，targetdir为解密文件存放目录
    bool decryptAESFile(std::string sourcefile, std::string targetfile , std::string password="123456");
};
