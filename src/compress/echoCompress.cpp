#include "../../include/compress/echoCompress.hpp"

 
// 编码函数
void EchoCompress::encode(haffNode *pn, std::string code){
    pn->code = code;
    if (pn->left)
...
// 压缩函数
bool EchoCompress::compress(std::string sourcefile, std::string targetfile){
    codeMap.clear();
    std::ifstream inFile;
    std::ofstream outFile;
  
    inFile.open(sourcefile, std::ios::in | std::ios::binary);
    if (!inFile) return false; // 打开源文件失败
    outFile.open(targetfile, std::ios::out | std::ios::binary);
    if (!outFile){
        inFile.close();
...
// 解压部分
void EchoCompress::creat_tree(haffNode *father, unsigned char uchar, std::string code){
    if (code.empty()){
        father->uchar = uchar;
        return;
    }
...
bool EchoCompress::decompress(std::string sourcePath, std::string targetfile){
    // 打开源文件
    if (sourcePath.substr(sourcePath.find_last_of(".") ) != BUKindsSuffix["Cmp"]){
        std::cout << "源文件格式不正确\n";
        return false; 
    }
    std::ifstream inFile;
